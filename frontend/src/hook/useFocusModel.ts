...
export const useFocusAI = () => {
  ...
  const modelRef =
    useRef<tmImage.CustomMobileNet | null>(
      null
    );
  ...
  const [notFocusScore, setNotFocusScore] =
    useState(0);
  ...
  const predict = async (
    ...
    focusEMA.current = ema(
      focusEMA.current,
      focus,
      ALPHA
    );

    notFocusEMA.current = ema(
      notFocusEMA.current,
      notFocus,
      ALPHA
    );
    ...
    if (
      ...
      setFocusScore(
        Number((f * 100).toFixed(1))
      );

      setNotFocusScore(
        Number((nf * 100).toFixed(1))
      );
    }
